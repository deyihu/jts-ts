 ...
 * @version 1.7
 */
export class CoordinateList extends Array {
     ...
     */
    public constructor(coord?: Array<Coordinate>, allowRepeated?: boolean) {
        // ensureCapacity(coord.length);
        super();
        if (allowRepeated === undefined) {
            allowRepeated = true;
        }
        this.add(coord, allowRepeated);
    }

    public getCoordinate(i: number) {
        return this[i];
    }

    public add(coord: Array<Coordinate> | Coordinate | number | undefined, allowRepeated: boolean | Object, start?: number | boolean, end?: number): boolean | undefined {
        if (coord === undefined) {
            return false;
        }
        //object
        if (typeof coord === 'object') {
            if (allowRepeated === false) {
                if (this.length > 0) {
        ...
        if (typeof coord === 'number' && typeof allowRepeated === 'object') {
            let i = coord;
     ...
     * @return true (as by general collection contract)
     */
    public addAll(coll: Array<Coordinate>, allowRepeated: boolean): boolean {
        let isChanged = false;
        coll.forEach(i => {
            this.add(i, allowRepeated);
            isChanged = true;
        });
     ...
     */
    public toCoordinateArray(isForward?: boolean): Array<Coordinate> {
     ...
     */
    public clone(): Object {
        const clone = new CoordinateList();
