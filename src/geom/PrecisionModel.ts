...
import { isUndefined, NumberUtil } from "../util/NumberUtil";
...
class Type {
    ...
    private static nameToTypeMap: Map<string, Type> = new Map();

    public constructor(name: string) {
        this.name = name;
        Type.nameToTypeMap.set(name, this);
    }
     ...
     */
    private readResolve(): Object | undefined {
...
export class PrecisionModel {
     ...
     */
    public static mostPrecise(pm1: PrecisionModel, pm2: PrecisionModel): PrecisionModel {
     ...
     */
    public static FIXED: Type = new PrecisionModel.Type("FIXED");
    /**
     * Floating precision corresponds to the standard Java
     * double-precision floating-point representation, which is
     * based on the IEEE-754 standard
     */
    public static FLOATING: Type = new PrecisionModel.Type("FLOATING");
    /**
     * Floating single precision corresponds to the standard Java
     * single-precision floating-point representation, which is
     * based on the IEEE-754 standard
     */
    public static FLOATING_SINGLE: Type = new PrecisionModel.Type("FLOATING SINGLE");
     ...
     */
    public static maximumPreciseValue: number = 9007199254740992.0;
    ...
    public constructor(modelType?: Type) {
        if (isUndefined(modelType)) {
            this.modelType = PrecisionModel.FLOATING;
        } else {
            this.modelType = (modelType as Type);
    ...
    public getMaximumSignificantDigits(): number {
        ...
        if (modelType == PrecisionModel.FLOATING) {
            maxSigDigits = 16;
        } else if (modelType == PrecisionModel.FLOATING_SINGLE) {
            maxSigDigits = 6;
        } else if (modelType == PrecisionModel.FIXED) {
            // maxSigDigits = 1 + (int) Math.ceil(Math.log(getScale()) / Math.log(10));
            maxSigDigits = 1 + parseInt(Math.ceil(Math.log(this.getScale()) / Math.log(10)) + '');
        }
        return maxSigDigits;
    }
    ...
    private setScale(scale: number) {
         ...
         */
        if (scale < 0) {
            this._gridSize = Math.abs(scale);
            this.scale = 1.0 / this._gridSize;
        }
        else {
     ...
     */
    public toInternal(external: Coordinate, internal: Coordinate) {
        if (this.isFloating()) {
     ...
     * @deprecated use makePrecise instead
     */
    public toInternal(external: Coordinate): Coordinate {
        const internal = new Coordinate(external);
     ...
     */
    public toExternal(internal: Coordinate): Coordinate {
        const external = new Coordinate(internal);
        return external;
    }
    ...
    public compareTo(o: Object): number {
        const other = (o as PrecisionModel);

        let sigDigits = this.getMaximumSignificantDigits();
        let otherSigDigits = other.getMaximumSignificantDigits();
        return NumberUtil.compare(sigDigits, otherSigDigits);
